...
AsyncLogging::AsyncLogging(const std::string& basename, std::size_t rollSize,
...
void AsyncLogging::flush() { output->flush(); }
...
void AsyncLogging::threadFunc() {
  ...
  while (running_) {
    lock_.lock();
    cond_.wait_for(lock_, std::chrono::seconds(flushInterval_),
                   [this]() { return !running_ || !buffers_.empty(); });
