...
Logger::Impl::Impl(LogLevel level, int savedErrno, const SourceFile& file, int line)
    : time_(Timestamp()), stream_(), level_(level), line_(line), basename_(file) {
...
inline void Logger::Impl::finish() { stream_ << " - " << basename_ << ':' << line_ << '\n'; }
// -------------- Logger::Impl Ends --------------

// -------------- Logger Starts --------------
void defaultOutput(const char* msg, std::size_t len) { fwrite(msg, 1, len, stdout); }
void defaultFlush() {
  // fflush(stdout);
}
...
Logger::Logger(SourceFile file, int line) : impl_(Logger::INFO, 0, file, line) {}
